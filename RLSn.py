...
def fnc(ghyt):
    ...
    def dydt(t, par, p):
        ...
        q = rho(h) * v * v / 2
        g = g_acc(r)
        dvx = - g * (x/r)  - q * p.CD * p.Sm * (vx/v) / p.mass
        dvy = - g * (y/r)  - q * p.CD * p.Sm  * (vy/v) / p.mass 
        dy = vy
        dx = vx
        return (dy, dx, dvy, dvx)

    def event_h_eq_0(t, par):
        return (par[0]**2+par[1]**2)**0.5 - Re+5

    event_h_eq_0.direction = -1
    event_h_eq_0.terminal = True
    print(100)
    sol = integrate.solve_ivp(lambda t, par: dydt(t, par, params), [0, t], [Re + h0, 0, vy, vx], method='BDF',
    ...
    ax.plot(sol.t, (sol.p[0] - Re))
    ax1.plot(sol.p[3], (sol.p[0] - Re))  #
    ax.minorticks_on()
    ax1.minorticks_on()
    ax2.minorticks_on()
